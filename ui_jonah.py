...
if using_gpiod:
    try:
        ...
        kill_line.request(
            ...
            default_vals=[1]  # Default to HIGH (safe/charging) state
        )

        def set_kill_switch(value):
            # value=True means activate killswitch (UNSAFE)
            # For active LOW relay, we need to set line LOW when killswitch is active
            kill_line.set_value(0 if value else 1)
    except Exception as e:
        print(f"ERROR: gpiod setup failed: {e}")
        print("Continuing with dummy GPIO...")
        
        def set_kill_switch(value):
            print(f"[DUMMY] Setting kill switch to {'ACTIVE' if value else 'INACTIVE'}")

else:
    try:
        print(f"Setting up RPi.GPIO on pin {KILL_PIN}...")
        GPIO.setup(KILL_PIN, GPIO.OUT)
        GPIO.output(KILL_PIN, GPIO.HIGH)  # Default to HIGH (safe/charging) state

        def set_kill_switch(value):
            # value=True means activate killswitch (UNSAFE)
            # For active LOW relay, we need to set line LOW when killswitch is active
            GPIO.output(KILL_PIN, GPIO.LOW if value else GPIO.HIGH)
    except Exception as e:
...
class BatteryManagementAI:
    ...
    def estimate_soh(self, voltage, current):
        """
        Estimate the State of Health (SOH) of the battery more realistically.
        Returns:
            SOH value (0-100%)
        """
        with self.readings_lock:
        ...
        try:
            ...
            # Only decay if voltage is consistently low
            if avg_voltage >= 13.0:
                decay_factor = 1.0  # Fully healthy
            elif avg_voltage >= 12.4:
                decay_factor = 0.998  # Nominal use
            elif avg_voltage >= 12.0:
                decay_factor = 0.995  # Low
            else:
                decay_factor = 0.99

            # Optional: adjust by internal resistance approximation
            voltage_factor = min(1.0, avg_voltage / nominal_voltage)
            adjusted_decay = decay_factor * (0.95 + 0.05 * voltage_factor)

            new_soh = self.soh_estimation * adjusted_decay

            # Clamp between 10 and 100
            new_soh = max(10.0, min(100.0, new_soh))
            self.soh_estimation = new_soh
    ...
    def load_models(self):
        ...
        try:
            ...
            if os.path.exists(soh_path):
                ...
                # only load if above floor, otherwise reset to 100%
                self.soh_estimation = val if val > 10.0 else 100.0
            else:
                self.soh_estimation = 100.0
            ...
            # import here so you can print full stack trace on error
            import traceback
            traceback.print_exc()
    ...
    def add_reading(self, voltage, current, temperature):
        ...
        try:
            ...
            with self.readings_lock:
                readings_length = len(self.readings)
                
            if readings_length >= 100 and readings_length % 100 == 0:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.ai_system = BatteryManagementAI(
            voltage_pin=2,
            current_pin=1,   # Changed from 4 to 1 to match the working code
            temp_pin=0,      # Changed from 3 to 0 to match the working code
    ...
    def update_readings(self):
        try:
            # --- read sensors ---
            r_t = self.read_raw(0)  # Changed from 3 to 0
            v_t = r_t / 1024.0 * 5.0
            t_c = 100*(v_t - 0.71) + 25
            t_f = t_c * 9/5 + 32
            self.temp_label.setText(f"{t_f:.1f} °F")

            r_i = self.read_raw(1)  # Changed from 4 to 1
            v_i = r_i / 1024.0 * 5.0
            i_a = (v_i - 2.46)/0.107
            ...
            # Changed to match the logic in working code - active LOW
            ...
            if current_time - self.last_csv_log_time >= self.csv_log_interval:
                self.write_csv_log(
                    timestamp=datetime.datetime.now(),
                    voltage=b_v,
                    current=i_a,
                    ...
                    killswitch_status=combined_red
                )
    ...
    def write_csv_log(self, timestamp, voltage, current, temperature, soc, soh, anomaly_score, is_anomaly, killswitch_status):
        """Write sensor data to CSV log file"""
        try:
            # Create filename using current date
            filename = os.path.join(self.csv_log_dir, f"battery_log_{timestamp.strftime('%Y-%m-%d')}.csv")
            file_exists = os.path.exists(filename)
            
            # Create or append to CSV file
            with open(filename, 'a', newline='') as csvfile:
                fieldnames = ['timestamp', 'voltage', 'current', 'temperature', 'soc', 'soh', 
                             'anomaly_score', 'is_anomaly', 'killswitch_status']
                ...
                writer.writerow({
                    ...
                    'killswitch_status': 1 if killswitch_status else 0
                })
            ...
            import traceback
            traceback.print_exc()
            self.log_event(f"Error writing to CSV log: {e}")
    ...
    def closeEvent(self, event):
        ...
        try:
            # Reset kill switch - set to inactive (HIGH for active-LOW relay)
