...
class BatteryCanvas(FigureCanvas):
    def __init__(self, parent=None, width=6, height=3, dpi=100):
        ...
        # plot line
        self.line, = self.axes.plot(self.time_history,
                                   self.soc_history,
                                   "-", linewidth=2)
    ...
    def update_soc(self, soc, dt):
        ...
        self.axes.set_xlim(self.time_history[0],
                           self.time_history[-1] * 1.05)
...
class MainWindow(QMainWindow):
    ...
    def init_spi(self):
        ...
        # pack voltage → SOC mapping
        self.volt_min = 11.2  # 0% SOC
        self.volt_max = 14.6  # 100% SOC

        # safety thresholds
        self.RED_TEMP    = 75.0   # °C
        self.RED_CURRENT = 7.0    # A
        self.RED_VOLTAGE = self.volt_max  # V

    def init_gpio(self):
        # kill‑switch on BCM26, start HIGH (safe)
    ...
    def update_readings(self):
        ...
        # SOC
        soc = (batt_v - self.volt_min) \
              / (self.volt_max - self.volt_min) * 100.0
        soc = max(0.0, min(100.0, soc))
        self.battery_canvas.update_soc(soc, dt=1)

        # safety kill‑switch
        red = (temp_c > self.RED_TEMP
               or current > self.RED_CURRENT
               or batt_v > self.RED_VOLTAGE)
        if red:
