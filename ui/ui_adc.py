...
class MainWindow(QMainWindow):
    ...
    def update_readings(self):
        ...
        #safety
        red_trip=(t_c>self.RED_TEMP or i_a>self.RED_CURRENT or b_v>self.RED_VOLTAGE)
        ...
        if red_trip:
            ...
            GPIO.output(26,GPIO.HIGH)
        def status(val,buf,m,r):
            if val>r:
                return 'RED','color:red;'
            if len(buf)==buf.maxlen and sum(buf)/len(buf)>m:
                return 'YELLOW','color:orange;'
            return 'GREEN','color:green;'
        s_t,c_t=status(t_c,self.buf_t,self.MAX_TEMP,self.RED_TEMP)
        s_i,c_i=status(i_a,self.buf_i,self.MAX_CURRENT,self.RED_CURRENT)
        s_v,c_v=status(b_v,self.buf_v,self.MAX_VOLTAGE,self.RED_VOLTAGE)
        self.temp_status.setText(s_t);self.temp_status.setStyleSheet(c_t)
        self.current_status.setText(s_i);self.current_status.setStyleSheet(c_i)
